(() => {
  ...
  function renderAttachments(){const list=$('attachment-list');list.replaceChildren();selectedAttachments.forEach((file,index)=>{const li=document.createElement('li');li.textContent=file.name+' ('+(file.size/1024).toFixed(0)+' KB) ';const remove=document.createElement('button');remove.type='button';remove.textContent='Remover';remove.onclick=()=>{selectedAttachments.splice(index,1);renderAttachments()};li.append(remove);list.append(li)});}
  $('attachments').onchange=()=>{const files=[...selectedAttachments,...$('attachments').files];if(files.length>10||files.reduce((n,f)=>n+f.size,0)>10*1024*1024){notify('Use no máximo 10 anexos, somando até 10 MB.','error');}else{selectedAttachments=files;renderAttachments();}$('attachments').value='';};
  function compose(reply = null) {
    selectedAttachments=[]; renderAttachments(); $('attachments').value=''; state.replyToId = reply?.id || null; $('dialog-title').textContent = reply ? 'Responder e-mail' : 'Novo e-mail'; $('client-field').style.display = reply ? 'none' : 'grid'; $('to').value = reply ? (reply.direction === 'received' ? reply.fromEmail : reply.toEmail) : ''; $('subject').value = reply ? `Re: ${String(reply.subject || '').replace(/^(re:\s*)+/i,'')}` : ''; $('message').value = ''; $('modal').classList.add('open'); setTimeout(() => (reply ? $('message') : $('client')).focus(), 50);
  }
  async function init() {
    try {
  ...
  $('form').onsubmit = async event => { event.preventDefault(); const send=$('send'); send.disabled=true; send.textContent='Enviando…'; try { const attachments=await Promise.all(selectedAttachments.map(file=>new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve({filename:file.name,content:String(reader.result).split(',')[1]});reader.onerror=()=>reject(Error('Não foi possível ler '+file.name));reader.readAsDataURL(file);}))); await api('agent.sendMailboxEmail',{attachments,replyToId:state.replyToId||undefined,clientId:state.replyToId?undefined:Number($('client').value),subject:$('subject').value,body:$('message').value},true); $('modal').classList.remove('open'); notify('E-mail enviado e registrado no portal.'); state.folder='sent'; document.querySelectorAll('[data-folder]').forEach(item=>item.classList.toggle('primary',item.dataset.folder==='sent')); await load(); } catch(error){notify(error.message,'error')} finally{send.disabled=false;send.textContent='Enviar'} };
  init();
})();
