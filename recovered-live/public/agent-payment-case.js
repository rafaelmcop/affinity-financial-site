(() => {
  ...
  async function load() {
    try {
      ...
      content.innerHTML = `<section class="card"><span class="status ${statusClass}">${statusText}</span><h2>${esc(candidate.name || data.client?.name || "Pagamento pendente")}</h2><p class="muted">${esc(data.notice?.actionDetail || "Revise os dados encontrados e tome a ação necessária.")}</p><div class="grid"><div><b>Cliente</b>${esc(candidate.name || "Ainda não identificado")}</div><div><b>Apólice</b>${esc(candidate.policyNumber || "Ainda não identificada")}</div><div><b>Telefone</b>${esc(phone || "Não informado")}</div><div><b>E-mail</b>${esc(candidate.email || "Não informado")}</div><div><b>Produto</b>${esc(data.policy?.product || "Não informado")}</div><div><b>Status</b>${esc(data.policy?.status || "Não informado")}</div><div><b>Data da aplicação</b>${esc(data.policy?.issuedAt || "Não informada")}</div><div><b>Premium mensal</b>${data.policy ? money(data.policy.premiumAmount) : "Não informado"}</div><div><b>Target premium anual</b>${data.policy ? money(data.policy.targetPremium) : "Não informado"}</div><div><b>Cobertura</b>${data.policy ? money(data.policy.coverageAmount) : "Não informada"}</div><div><b>Beneficiários</b>${esc(data.policy?.beneficiaries || "Não informados")}</div></div><div class="actions">${data.client ? `<a class="primary" href="/agentes/clientes?cliente=${data.client.id}&conversa=1">Abrir ficha completa do cliente</a>` : ""}${data.policy ? `<a href="/agentes/apolices?busca=${encodeURIComponent(data.policy.policyNumber || "")}">Abrir esta apólice</a>` : ""}<button id="resolve">Marcar como resolvido</button><button id="delete">Excluir pendência</button></div></section>
