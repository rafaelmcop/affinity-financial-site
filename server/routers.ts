...
function parseAmericanBirthDate(value?: string) {
  ...
  if (!match)
    throw new Error("Use a data de aniversário no formato MM/DD/AAAA");
...
const notificationRouter = router({
  ...
  sendAffiliateApprovalEmail: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const success = await sendAffiliateApprovalEmail(
        input.email,
        input.name,
        input.affiliateCode
      );
  ...
  sendPolicyApprovalEmail: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const success = await sendPolicyApprovalEmail(
        input.email,
        input.clientName,
        input.policyNumber,
        input.points
  ...
  sendCommissionCreditEmail: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const success = await sendCommissionCreditEmail(
        input.email,
        input.name,
        input.amount,
        input.policyNumber
...
export const appRouter = router({
  ...
  auth: router({
    ...
    logout: publicProcedure.mutation(({ ctx }) => {
      ...
      ctx.res.clearCookie("affinity_admin_session", {
        ...cookieOptions,
        maxAge: -1,
      });
      ctx.res.clearCookie("affinity_affiliate_session", {
        ...cookieOptions,
        maxAge: -1,
      });
      return {
  ...
  affiliate: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (
          !affiliate ||
          !verifyPassword(input.password, affiliate.passwordHash)
        ) {
        ...
        const { createAffiliateSession, AFFILIATE_SESSION_COOKIE } =
      ...
      .query(async ({ input, ctx }) => {
        ...
        const stats = {
          ...
          totalCommission: referrals
            ...
            .reduce(
              (sum, r) =>
                sum + parseFloat(r.commissionAmount?.toString() || "0"),
              0
            ),
        };
    ...
    submitLead: affiliateProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await createAffiliateReferral({
          ...
          referralCode: `LEAD-${Date.now().toString(36)}`,
  ...
  agent: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [account] = await db
          .select()
          .from(adminAccounts)
          .where(eq(adminAccounts.email, input.email.toLowerCase()))
          .limit(1);
        if (
          !account ||
          ...
          !(await (
            await import("bcryptjs")
          ).compare(input.password, account.passwordHash))
        )
        ...
        ctx.res.cookie(
          (await import("./sessionAuth")).ADMIN_SESSION_COOKIE,
          token,
          { ...getSessionCookieOptions(ctx.req), maxAge: 8 * 60 * 60 * 1000 }
        );
        return {
          id: account.id,
          email: account.email,
    ...
    dashboard: adminProcedure.query(async ({ ctx }) => {
      ...
      const policies = await db
        .select()
        .from(agentPolicies)
        .where(eq(agentPolicies.agentEmail, email));
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        let [client] = input.clientEmail
          ? await db
              ...
              .where(
                and(
                  eq(crmClients.email, input.clientEmail.toLowerCase()),
    ...
    listTasks: adminProcedure.query(async ({ ctx }) => {
      ...
      return db
        .select()
        .from(agentTasks)
        .where(eq(agentTasks.agentEmail, ctx.adminEmail.toLowerCase()));
    }),
    ...
    toggleTask: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db
          .update(agentTasks)
          .set({ status: input.completed ? "completed" : "pending" })
          .where(
            and(
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const owner = ctx.adminEmail.toLowerCase(),
          [current] = await db
            .select()
            .from(agentEmailSettings)
            .where(eq(agentEmailSettings.agentEmail, owner))
            .limit(1);
        const clear = (input.password || "").replace(/\s/g, "");
        const password = clear
          ? await (
              await import("../worker/cloudflare-email")
            ).encryptSmtpPassword(
              clear,
              process.env.JWT_SECRET || "local-preview-secret"
            )
          : current?.password;
        if (!password)
          throw new Error("Informe a senha específica de aplicativo");
        const values = {
          ...input,
          agentEmail: owner,
        ...
        if (current)
          await db
            ...
            .set(values)
            .where(eq(agentEmailSettings.id, current.id));
        else await db.insert(agentEmailSettings).values(values);
        await db
          .update(adminAccounts)
          .set({ contactEmail: input.fromEmail })
          .where(eq(adminAccounts.email, owner));
        return { success: true };
      }),
    testEmailSettings: adminProcedure
  ...
  admin: router({
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const hashMatches = adminPasswordHash
          ? await (
              await import("bcryptjs")
            ).compare(input.password, adminPasswordHash)
          : false;
        ...
        if (
          ...
          (!hashMatches && !temporaryPasswordMatches)
        ) {
          throw new Error("Credenciais inválidas");
        }
    ...
    getStats: adminProcedure.query(async () => {
      const { getAdminStats, getPoliciesByStatus, getCommissionsByAffiliate } =
        await import("./db");
    ...
    saveEmailConfig: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const password = input.password
          ? encryptSecret(input.password.replace(/\s/g, ""))
          : current?.password;
        if (!password)
          throw new Error("Informe a senha específica de aplicativo");
    ...
    testEmailConfig: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!(await sendTestEmail(input.email)))
          throw new Error(
    ...
    getPoliciesPending: adminProcedure.query(async () => {
    ...
    approvePolicyAdmin: adminProcedure
      ...
      .mutation(async ({ input }) => {
        const { updatePolicyStatus, getPolicyById, getAffiliateById } =
          await import("./db");
    ...
    rejectPolicyAdmin: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (affiliate) {
          ...
          await sendPolicyRejectionEmail(
            affiliate.email,
            policy.clientName,
            policy.policyNumber
          );
    ...
    addPolicy: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const existing = await db
          .select()
          .from(policies)
          .where(eq(policies.policyNumber, input.policyNumber))
          .limit(1);
    ...
    getPendingAffiliates: adminProcedure.query(async () => {
    ...
    updateAffiliateEmail: adminProcedure
      .input(
        z.object({ affiliateId: z.number(), newEmail: z.string().email() })
      )
      .mutation(async ({ input }) => {
    ...
    resetAffiliatePasswordByAdmin: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await resetAffiliatePasswordByAdmin(
          input.affiliateId,
          input.newPassword
        );
    ...
    updateAffiliateLead: adminProcedure
      .input(
        z.object({
          id: z.number(),
      ...
      .mutation(async ({ input }) => {
        ...
        await db
          ...
          .where(eq(affiliateReferrals.id, input.id));
        return { success: true };
      }),
    ...
    listAdmins: adminProcedure.query(async ({ ctx }) => {
      ...
      const [currentAccount] = await db
        .select()
        .from(adminAccounts)
        .where(eq(adminAccounts.email, currentEmail))
        .limit(1);
      const currentRole =
        currentEmail === process.env.ADMIN_EMAIL?.toLowerCase()
          ? "master"
          : (currentAccount?.adminRole ?? "standard");
      ...
      .input(
        z
          .object({
            ...
            accessAgent: z.boolean(),
            accessAffiliate: z.boolean(),
            adminRole: z.enum(["master", "standard"]),
            password: strongPassword,
          })
          .refine(
            value =>
              value.accessAdmin || value.accessAgent || value.accessAffiliate,
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.transaction(async tx => {
          if (input.accessAdmin || input.accessAgent) {
            const alreadyAdmin =
              existingInternal?.accountType === "admin" ||
              existingInternal?.accountType === "both";
            const alreadyAgent =
              existingInternal?.accountType === "agent" ||
              existingInternal?.accountType === "both";
            const accessAdmin = input.accessAdmin || alreadyAdmin,
              accessAgent = input.accessAgent || alreadyAgent;
            const accountType =
            ...
            if (existingInternal)
              await tx
                ...
                .set({
                  accountType,
                  adminRole:
                    existingInternal.adminRole === "master"
                      ? "master"
                      : accessAdmin
                        ? input.adminRole
                        : "standard",
                ...
                .where(eq(adminAccounts.id, existingInternal.id));
            else
              await tx.insert(adminAccounts).values({
                ...
                accountType,
                adminRole: input.accessAdmin ? input.adminRole : "standard",
                passwordHash: existingAffiliate?.passwordHash || passwordHash,
                isActive: 1,
              });
          }
          if (input.accessAffiliate) {
            if (existingAffiliate)
              await tx
                .update(affiliates)
                .set({ isActive: 1, status: "approved" })
              ...
              await tx.insert(affiliates).values({
                ...
                name: input.name,
                phone: input.phone || null,
                passwordHash: existingInternal?.passwordHash || passwordHash,
        ...
        return {
          success: true,
          updatedExisting: Boolean(existingInternal || existingAffiliate),
        };
      }),
    ...
    updateAffiliateCategories: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [internal] = await db
          .select()
          .from(adminAccounts)
          .where(eq(adminAccounts.email, affiliate.email.toLowerCase()))
          .limit(1);
        await db
          .update(affiliates)
          .set({ isActive: input.accessAffiliate ? 1 : 0 })
          .where(eq(affiliates.id, affiliate.id));
        if (!input.accessAdmin && !input.accessAgent) {
          if (internal?.email.toLowerCase() === actorEmail)
            throw new Error("Você não pode remover seu próprio acesso");
          if (internal)
            await db
              .update(adminAccounts)
              .set({ isActive: 0 })
              .where(eq(adminAccounts.id, internal.id));
          return { success: true };
        }
        const accountType =
          input.accessAdmin && input.accessAgent
            ? "both"
            : input.accessAdmin
              ? "admin"
              : "agent";
    ...
    setInternalPortalAccess: adminProcedure
      .input(
        z
          ...
          .refine(value => value.accessAdmin || value.accessAgent, {
            message: "Escolha pelo menos um portal",
          })
      )
      .mutation(async ({ input, ctx }) => {
        const { getDb } = await import("./db");
    ...
    createAdmin: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (existing) {
          const admin =
            ["admin", "both"].includes(existing.accountType) ||
            ["admin", "both"].includes(input.accountType);
          const agent =
            ["agent", "both"].includes(existing.accountType) ||
            ["agent", "both"].includes(input.accountType);
          await db
            .update(adminAccounts)
            .set({
              accountType: admin && agent ? "both" : admin ? "admin" : "agent",
              isActive: 1,
            })
    ...
    updateAdmin: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (actorRole !== "master" && !isSelf)
          throw new Error(
            "Administrador padrão só pode alterar a própria conta"
          );
        if (actorRole !== "master" && input.adminRole !== target.adminRole)
          throw new Error(
        ...
        if (target.adminRole === "master" && input.adminRole !== "master") {
          const masters = await db
            ...
            .where(
              and(
                eq(adminAccounts.adminRole, "master"),
        ...
        return {
          success: true,
          emailChanged:
            isSelf && target.email.toLowerCase() !== input.email.toLowerCase(),
        };
      }),
    ...
    changeMyPassword: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!envPasswordMatches && !accountPasswordMatches)
    ...
    setAdminActive: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [target] = await db
          ...
          .where(eq(adminAccounts.id, input.id))
          .limit(1);
        if (target?.email === ctx.adminEmail)
  ...
  crm: router({
    ...
    addActivity: adminProcedure
      .input(
        z.object({
          clientId: z.number(),
          type: z.enum(["note", "call", "email", "sms", "whatsapp", "status"]),
  ...
  passwordReset: router({
    ...
    resetPassword: publicProcedure
      .input(
        z.object({ token: z.string().min(32), newPassword: strongPassword })
      )
      .mutation(async ({ input, ctx }) => {
        enforceRateLimit(
          ctx.req,
    ...
    update: adminProcedure
      ...
      .mutation(async ({ input }) => {
        if (
          ...
          input.mediaType &&
          !isValidMediaUrl(input.mediaUrl, input.mediaType)
        ) {
          throw new Error("O endereço da mídia não é válido.");
        }
        ...
        const updateData =
          amountReceived === undefined
            ? data
            : { ...data, amountReceived: amountReceived.toFixed(2) };
