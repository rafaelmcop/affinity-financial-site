...
export default function AdminCommunicationAudit() {
  ...
  const auditConversations = Array.isArray(audit.data?.conversations) ? audit.data.conversations : [];
  const auditCampaigns = Array.isArray(audit.data?.campaigns) ? audit.data.campaigns : [];
  const internalMessages = Array.isArray(internal.data) ? internal.data : [];
  const agentNames = useMemo(() => new Map(agentList.map(item => [item.email.toLowerCase(), item.name])), [agentList]);
  const text = search.trim().toLowerCase();
  const conversations = auditConversations.filter(item =>
  ...
  const campaigns = auditCampaigns.filter(item =>
    (agentFilter === "all" || item.agentEmail.toLowerCase() === agentFilter) &&
  ...
  return <div className="min-h-screen bg-black text-white lg:pl-64"><AdminSidebar /><main className="mx-auto max-w-6xl space-y-6 px-4 py-8">
    ...
    <Card className="grid gap-3 border-gold/20 bg-[#0b1524] p-4 md:grid-cols-[1fr_15rem]"><label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-gray-500" /><Input className="pl-9" placeholder="Buscar cliente, assunto ou conteúdo" value={search} onChange={e => setSearch(e.target.value)} /></label><select className="h-10 rounded-md border border-white/20 bg-black px-3" value={agentFilter} onChange={e => setAgentFilter(e.target.value)}><option value="all">Todos os agentes</option>{agentList.filter(item => ["agent", "both"].includes((item as any).accountType || "agent")).map(item => <option key={item.id} value={item.email.toLowerCase()}>{item.name}</option>)}</select></Card>
