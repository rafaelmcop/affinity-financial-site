...
export default function AdminUsers() {
  ...
  const saveNew = async (event: React.FormEvent) => {
    ...
    try {
      await createUser.mutateAsync({ ...newUser, accessAdmin: newUser.accountType === 'admin' || newUser.accountType === 'both', accessAgent: newUser.accountType === 'agent' || newUser.accountType === 'both', accessAffiliate: newUser.accessAffiliate });
      setNewUser(emptyUser); setCreating(false); await Promise.all([admins.refetch(), affiliates.refetch()]); toast.success('Usuário criado com os acessos selecionados');
    }
  ...
  return <div className="min-h-screen bg-black text-white lg:pl-64">
    ...
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
      ...
      {creating && <Card className="border-gold/30 bg-[#101b2b] p-6"><FormTitle title="Novo usuário interno" close={() => setCreating(false)} /><form onSubmit={saveNew} className="grid gap-4 md:grid-cols-2">
        ...
        <Input placeholder="WhatsApp" value={newUser.whatsapp} onChange={e => setNewUser({ ...newUser, whatsapp: e.target.value })} /><Input type="password" placeholder="Senha forte" value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} required />
...
function Badges({ user }: { user: any }) { if (user.kind === 'affiliate') return <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-bold text-emerald-300">Afiliado</span>; return <>{(user.accountType === 'admin' || user.accountType === 'both') && <span className="rounded-full bg-blue-500/15 px-3 py-1 text-xs font-bold text-blue-300">Administrador</span>}{(user.accountType === 'agent' || user.accountType === 'both') && <span className="rounded-full bg-purple-500/15 px-3 py-1 text-xs font-bold text-purple-300">Agente</span>}{user.adminRole === 'master' && <span className="rounded-full bg-gold/15 px-3 py-1 text-xs font-bold text-gold"><ShieldCheck className="mr-1 inline h-3 w-3" />Mestre</span>}</>; }
