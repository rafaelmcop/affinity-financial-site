...
import {preferenceToken} from '../recovered-live/worker/automation-preferences.js';
...
for(const file of ['automation-preferences.js','mail-attachments.js','five-rings-credits.js'])fs.copyFileSync(root+'/worker/'+file,temp+'/'+file);
...
try{
 ...
 if(fullPortal){
   ...
   const token=await preferenceToken('test-only-private-secret-should-never-be-deployed',{owner:'admin@brand.test',clientId:client.id,email:client.email,scope:'weekly_monday',label:'Segunda-feira'});
   const preferenceUrl=origin+'/email/preferences?token='+encodeURIComponent(token);
   assert.equal((await page.request.get(preferenceUrl)).status(),200);
   await call('agent.automationSubscriptions',{clientId:client.id});
   assert.equal((await db.prepare('SELECT COUNT(*) AS n FROM crmAutomationSubscriptions').first()).n,0);
   assert.equal((await page.request.post(preferenceUrl)).status(),200);
   const preferences=(await call('agent.automationSubscriptions',{clientId:client.id})).result.data.json;
   assert.equal(preferences[0].occasion,'weekly_monday');assert.equal(preferences[0].requestedBy,'client');assert.equal(preferences[0].isActive,0);
   assert.equal((await call('agent.setAutomationSubscription',{clientId:client.id,occasion:'weekly_monday',isActive:true})).result.data.json.success,true);
   assert.equal((await db.prepare('SELECT isActive FROM crmAutomationSubscriptions').first()).isActive,1);
   const foreign=await db.prepare("INSERT INTO crmClients(name,email,assignedAdminEmail,status) VALUES('Other','other@example.test','agent@brand.test','new')").run();
   assert((await call('agent.setAutomationSubscription',{clientId:Number(foreign.meta.last_row_id),occasion:'weekly_monday',isActive:false})).error);
   console.log('PASS unsubscribe GET does not mutate; POST scopes preference; owner can reactivate; foreign client rejected');
