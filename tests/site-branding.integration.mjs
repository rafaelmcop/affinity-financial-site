...
const schema=fs.readFileSync('/tmp/independent-portal-schema.sql','utf8');for(const statement of schema.split(';').map(x=>x.trim()).filter(Boolean))await db.prepare(statement).run();
